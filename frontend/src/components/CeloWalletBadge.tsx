...
import { useUsdcBalance } from "@/lib/celo/payments";
...
export function CeloWalletBadge({ compact, showBalance = true }: Props) {
  ...
  const { data: balance, isFetching, refetch } = useUsdcBalance();
  ...
  if (compact) {
    return (
      <a
        ...
        title={showBalance ? formatUsdc(balance) : undefined}
      >
      ...
      {showBalance && (
        <div className="mt-4 flex items-end justify-between">
          <div>
            ...
            <div className="font-display text-2xl font-bold text-emerald-700">{formatUsdc(balance)}</div>
          </div>
