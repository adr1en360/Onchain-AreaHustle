  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      if (paymentMode === "onchain" && created.escrow?.task_ref) {
        toast.info("Approve NGNm and lock escrow in your wallet…");
    ...
    <div className="mx-auto max-w-3xl px-4 py-14 sm:py-20">
      <div className="text-center mb-10">
        ...
        <p className="text-muted-foreground mt-3 max-w-md mx-auto">
          {canPayOnchain
            ? "Escrow locks in NGNm on Celo when you publish. Hustlers get paid automatically on release."
            : "Aethex listens. Gemini structures. Lock escrow when it looks right."}
        </p>
        {celoEnabled && !canPayOnchain && (
          <div className="mt-6 flex flex-col items-center gap-3">
            <p className="text-sm text-amber-700">Connect and link your Celo wallet to pay with NGNm escrow.</p>
            <WalletConnectButton />
          </div>
        )}
        {canPayOnchain && (
              ...
              <div className="mt-10 animate-fade-up">
                ...
                >
                  {phase === "locked" ? (
                    ...
                    <>
                      <Lock className="h-4 w-4" /> {canPayOnchain ? "Lock NGNm escrow on Celo" : "Lock Escrow to Confirm"}
                    </>
                  )}
                </button>
              </div>
