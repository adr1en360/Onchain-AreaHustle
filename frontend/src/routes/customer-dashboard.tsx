...
function CustomerDashboard() {
  ...
  const confirmMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      toast.success(data?.onchain ? "NGNm payment released on Celo!" : "Payment released! Escrow funds transferred to Hustler.");
                  ...
                  {(status === "matched" || status === "accepted" || status === "in_progress" || status === "active") && (
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {job.payment_mode === "onchain" && job.escrow_status === "assigned" && (
                        ...
                        >
                          <CheckCircle className="h-3.5 w-3.5" /> Release NGNm on Celo
                        </button>
                      )}
                    </div>
                  )}
                  {status === "awaiting_confirmation" && (
                    <div className="flex flex-col items-end gap-2 w-full sm:w-auto">
                      <span className="text-xs text-orange-600 font-semibold italic px-1">
                        ...
                        {job.payment_mode === "onchain" && " Release NGNm from your wallet."}
                      </span>
                      <button
                        onClick={() => handleConfirm(job.id || job._id)}
                        disabled={confirmMutation.isPending}
                      ...
                      >
                        <CheckCircle className="h-4 w-4" /> {job.payment_mode === "onchain" ? "Release NGNm on Celo" : "Release Payment"}
                      </button>
                    </div>
                  )}
                  {(status === "done" || status === "completed") && (
